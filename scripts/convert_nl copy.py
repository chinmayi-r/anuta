...
class LogicToNaturalLanguageConverter:
    def __init__(self):
        ...
        self.value_mappings = {
            ...
            'hasflags': 'set',
            'noflags': 'not set',
            'uninterested': 'dynamic ports',  # Changed from 'any value'
            'ICMP': 'ICMP',
            'TCP': 'TCP',
            'UDP': 'UDP'
        }
    ...
    def collect_or_conditions(self, expr: Dict, field: str) -> List[str]:
        """Collect all OR conditions for the same field to create set notation."""
        if expr['type'] == 'predicate':
            if expr['field'] == field and expr['operator'] == '=':
                return [expr['value']]
            return []
        elif expr['type'] == 'binary_op' and expr['operator'] == '∨':
            left_values = self.collect_or_conditions(expr['left'], field)
            right_values = self.collect_or_conditions(expr['right'], field)
            return left_values + right_values
        return []
    
    def is_same_field_or_chain(self, expr: Dict) -> Tuple[bool, str, List[str]]:
        """Check if this is a chain of OR conditions for the same field with = operator."""
        if expr['type'] != 'binary_op' or expr['operator'] != '∨':
            return False, "", []
        
        # Get all predicates in this OR chain
        predicates = self.get_all_or_predicates(expr)
        
        # Check if all predicates are for the same field with = operator
        if not predicates:
            return False, "", []
        
        first_field = predicates[0]['field']
        if all(p['field'] == first_field and p['operator'] == '=' for p in predicates):
            values = [p['value'] for p in predicates]
            return True, first_field, values
        
        return False, "", []
    
    def get_all_or_predicates(self, expr: Dict) -> List[Dict]:
        """Get all predicates connected by OR operators."""
        if expr['type'] == 'predicate':
            return [expr]
        elif expr['type'] == 'binary_op' and expr['operator'] == '∨':
            left_preds = self.get_all_or_predicates(expr['left'])
            right_preds = self.get_all_or_predicates(expr['right'])
    ...
    def predicate_to_text(self, predicate: Dict) -> str:
        ...
        # Handle "uninterested" values specially for ports
        if value == 'uninterested':
            if 'port' in field_text.lower():
                return f"the {field_text} is dynamic ports"
            else:
    ...
    def format_value_set(self, field: str, values: List[str]) -> str:
        """Format a list of values as a set notation for better readability."""
        field_text = self.field_mappings.get(field, field)
        ...
        for value in values:
            if value == 'uninterested':
                if 'port' in field_text.lower():
                    formatted_values.append('dynamic ports')
                else:
                    formatted_values.append('any value')
            else:
                # Remove articles for set notation
                formatted_value = self.value_mappings.get(value, value)
                if formatted_value.startswith('a '):
                    formatted_value = formatted_value[2:]
                elif formatted_value.startswith('an '):
                    formatted_value = formatted_value[3:]
                formatted_values.append(formatted_value)
        
        # Create set notation
        if len(formatted_values) == 1:
            return f"the {field_text} is {formatted_values[0]}"
        else:
            set_content = ', '.join(formatted_values)
            return f"the {field_text} is in {{{set_content}}}"
    ...
    def expression_to_text(self, expr: Dict) -> str:
        ...
        if expr['type'] == 'predicate':
            ...
            # Check if this is a same-field OR chain that can be converted to set notation
            if expr['operator'] == '∨':
                is_same_field, field, values = self.is_same_field_or_chain(expr)
                if is_same_field:
                    return self.format_value_set(field, values)
            ...
            if expr['operator'] == '∧':
                ...
                # For complex OR expressions, use parentheses for clarity
                if expr['left']['type'] == 'binary_op' or expr['right']['type'] == 'binary_op':
                    return f"({left_text}) or ({right_text})"
                else:
                    return f"{left_text} or {right_text}"
            elif expr['operator'] == '⇒':
                # Add parentheses around complex conditions for clarity
                if expr['left']['type'] == 'binary_op':
...
if __name__ == "__main__":
    ...
    converter = LogicToNaturalLanguageConverter()
    test_statement = "((Bytes > 982280) ∧ ((SrcPt = 0) v (SrcPt = 137) v (SrcPt = 138) v (SrcPt = uninterested))) ⇒ ((DstIpAddr = private_p2p))"
    
    print("Original statement:")
    print(test_statement)
    print("\nImproved translation:")
    result = converter.convert_to_natural_language(test_statement)
    print(result)
    print()
    
    # If running as main script with arguments, process file
    if len(sys.argv) > 1:
        main()